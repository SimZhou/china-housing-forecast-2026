...
DEFAULT_OUTPUT = REPO_ROOT / "data" / "processed" / "purchased" / "prefecture-housing-segment-trends.csv"
DEFAULT_FIGURE_DIR = REPO_ROOT / "reports" / "figures" / "purchased" / "prefecture-housing-segment-trends"
...
SCRIPT_REFERENCE = "生成脚本：src/build_prefecture_housing_segment_trends.py；城市细分：data/city-segment-seed-v1.csv"
METRIC_SLUGS = {
...
def build_rows(input_path: Path, segment_seed_path: Path) -> list[dict[str, str]]:
    ...
    for (metric_name, variable_id, unit, city_segment, year), item in sorted(
        grouped.items(),
        key=lambda pair: (pair[0][0], int(pair[0][4]), int(pair[1]["segment_rank"])),
    ):
        cities = item["cities"]
...
def points_for_series(
    ...
    for year, value in series:
        points.append(
            f"{scale_x(year, min_year, max_year, width):.1f},{scale_y(value, min_value, max_value, height):.1f}"
        )
    return " ".join(points)
...
def render_svg(metric_name: str, unit: str, rows: list[dict[str, str]], output_path: Path) -> None:
    ...
    series_by_segment: dict[str, list[tuple[int, float]]] = defaultdict(list)
    for row in metric_rows:
        series_by_segment[row["city_segment_v1"]].append((int(row["year"]), float(row["mean_value"])))

    all_points = [point for series in series_by_segment.values() for point in series]
    years = [year for year, _ in all_points]
    values = [value for _, value in all_points]
    min_year, max_year = min(years), max(years)
    min_value, max_value = min(values), max(values)

    title = f"{metric_name}：按城市细分组的均值趋势"
    ...
    lines: list[str] = [
        ...
        f'<text x="{PLOT_LEFT}" y="612" font-size="13" font-family="Arial, sans-serif">单位：{unit}；数值为购买数据衍生均值，仅本地探索使用</text>',
        f'<text x="{PLOT_LEFT}" y="636" font-size="12" font-family="Arial, sans-serif">{SOURCE_REFERENCE}</text>',
        f'<text x="{PLOT_LEFT}" y="658" font-size="12" font-family="Arial, sans-serif">{SOURCE_NOTE_REFERENCE}</text>',
        f'<text x="{PLOT_LEFT}" y="680" font-size="12" font-family="Arial, sans-serif">{SCRIPT_REFERENCE}</text>',
    ...
    for index, segment in enumerate(SEGMENT_ORDER):
        series = sorted(series_by_segment.get(segment, []))
        if len(series) < 2:
            continue
        ...
        lines.append(f'<text x="{legend_x + 36}" y="{y}" font-size="13" font-family="Arial, sans-serif">{segment}</text>')
...
def main() -> int:
    ...
    rows = build_rows(args.input.resolve(), args.segment_seed.resolve())
